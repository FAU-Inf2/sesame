	...
	private static MethodDeclaration lookupMatch(final BodyDeclaration decl,
		...
		if (decl instanceof AbstractTypeDeclaration) {
			...
			if (((classSpec != null) && (classSpecIndex < classSpec.size())
				...
				//if (classSpec != null && classSpecIndex + 1 >= classSpec.size()
				//		&& ((typeDecl instanceof TypeDeclaration && ((TypeDeclaration) typeDecl).isInterface())
				//			|| typeDecl instanceof AnnotationTypeDeclaration)) {
				//	throw new IllegalArgumentException("Cannot execute interface method");
				//}
